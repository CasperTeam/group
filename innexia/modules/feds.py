...
import innexia.modules.sql.feds_sql as sql
from innexia import (
    EVENT_LOGS,
    LOGGER,
    SUPPORT_CHAT,
...
from innexia.modules.helper_funcs.alternate import send_message
...
from innexia.modules.helper_funcs.string_handling import markdown_parser
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "innexiabot_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="innexiabot_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "innexiabot_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="innexiabot_fbanned_users.csv",
...
💞 /feduserhelp*:* Provides help for commands aninnexiabot can use
