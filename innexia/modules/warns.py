...
import telegram
from innexia import TIGERS, WOLVES, dispatcher
...
from innexia.modules.helper_funcs.extraction import (
    extract_text,
    extract_user,
    extract_user_and_text,
)
from innexia.modules.helper_funcs.filters import CustomFilters
from innexia.modules.helper_funcs.misc import split_message
from innexia.modules.helper_funcs.string_handling import split_quotes
from innexia.modules.log_channel import loggable
from innexia.modules.sql import warns_sql as sql
...
from innexia.modules.sql.approve_sql import is_approved
...
def button(update: Update, context: CallbackContext) -> str:
    ...
    if match:
        ...
        if res:
            ...
            update.effective_message.edit_text(
                "User has no warns.", parse_mode=ParseMode.HTML
            )
...
def warn_user(update: Update, context: CallbackContext) -> str:
    ...
    if user_id:
        ...
        message.reply_text("give correcr User ID to me idiot.")
    return ""
...
def set_warn_limit(update: Update, context: CallbackContext) -> str:
    ...
    if args:
        if args[0].isdigit():
            if int(args[0]) < 60:
                msg.reply_text("The minimum warn limit is 60!")
            else:
