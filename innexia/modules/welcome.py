...
import innexia.modules.sql.welcome_sql as sql
import innexia
from innexia import (
    DEV_USERS,
    LOGGER,
...
from innexia.modules.helper_funcs.chat_status import (
    is_user_ban_protected,
    user_admin,
)
from innexia.modules.helper_funcs.misc import build_keyboard, revert_buttons
from innexia.modules.helper_funcs.msg_types import get_welcome_type
from innexia.modules.helper_funcs.string_handling import (
...
def welcome(update: Update, context: CallbackContext):
    ...
    if not args or args[0].lower() == "noformat":
        ...
        if args[0].lower() in ("on", "yes"):
            ...
            update.effective_message.reply_text(
                "Okay! I'll send welcome message members when they join."
            )

        elif args[0].lower() in ("off", "no"):
            sql.set_welc_preference(str(chat.id), False)
            update.effective_message.reply_text(
                "I'll go loaf around and not welcome then."
            )
...
def user_button(update: Update, context: CallbackContext):
    ...
    if join_user == user.id:
        ...
        query.answer(text="OOps..! you are a human so you are unmuted!")
        bot.restrict_chat_member(
            chat.id,
            user.id,
            permissions=ChatPermissions(
        ...
        query.answer(text="Hey idiot who are you again stop touching me !")
...
*Admins only:*
 ...
 💞 /welcome*:* shows present welcome settings.
 💞 /welcome noformat*:* shows current welcome settings, without the formatting - useful to recycle your welcome messages!
 💞 /goodbye*:* same usage and args as `/welcome`.
 💞 /setwelcome <your message>*:* set a custom welcome message. If used replying to media, uses that media.
 💞 /setgoodbye <your message>*:* set a custom goodbye message. If used replying to media, uses that media.
